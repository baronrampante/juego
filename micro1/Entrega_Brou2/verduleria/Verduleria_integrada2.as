...
*/		
		...
		private function IniciarVariables():void {
			...
			_sys_CantClientesxSemana = 5;			  //--> Cantidad de clientes que comprende una semana (unidad de medida). Se incrementa con la popularidad
			...
			_sys_CantMaxClientes=2;						   //--> Cantidad máxima de clientes concurrentes en el stage (se va incrementando)
 	...
 	function fnQuitarPersonaje( vlnPersonaje:int, blnSeFue:Boolean):void {
		...
		private function fnFinAtiendeCliente( blnVentaEfectuada: Boolean ):void {
			...
			//Chequeamos la cantidad de clientes y verificamos cambio de semana
			...
			// Calculamos el dia que transcurre
			_sys_DiaCorriente = (_sys_SemanaCorriente * (_sys_TotalMeses*30))/_sys_TotalSemanas;
						...
						fnDebitarMercaderiaVitrina();
							...
							if (_sys_blnArregloCamion == true ) {
								 ...
								 mensaje_compra_realizada.gotoAndStop(2);
										  ...
										  // Se debita a la semana siguiente mercaderia
	...
	function fnVerificarDebitoMercaderiaComprada(): void {
			...
			if (_sys_DiaCorriente == _sys_SemanaDebitarMercaderia) {
				//_sys_DiaCorriente
